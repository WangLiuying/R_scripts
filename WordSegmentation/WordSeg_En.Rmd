...
```{r}
#思路：找到高频词，得到高频词的dtm_fw再按列加和获得总频数
freqword <- findFreqTerms(dtm,200)
dtm_fw <- DocumentTermMatrix(docs_map,
          control=list(dictionary=freqword))
dim(dtm_fw)
wordfreq <- apply(dtm_fw,2,sum)
wordfreq <- data.frame(word=names(wordfreq),Freq=wordfreq)
horse.fig <- "./horse1.png"
...
colnames(comparisonWordFreq) <- movie$title[1:10]
