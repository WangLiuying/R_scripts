...
#
library(ggplot2)
qplot(perp$Num_topics,perp$perplexity,geom = "line")
...
theta <- t(apply(gibbs@gamma+alpha,1,function(x) x/sum(x)))
#topic-word phi matrix
phi <- t(apply(gibbs@beta+delta,1,function(x) x/sum(x)))
...
dir.path <- "./LDAvis-gossip-version10"
